...
public class PolarLengthFinder extends WorldObject implements DrawableWorldObject, DrawScreenWorldObject, OneFingerReactWorldObject, LongPressReactWorldObject {
    private static final String CLASS_NAME = "PolarLengthFinder";
    ...
    public PolarLengthFinder(World world, int sampleCount, PolarFunctionObject functionObject) {
        super(world);
        this.theta0 = 0;
        this.theta1 = 0;
    ...
    public void draw(WorldView view, Paint paint, Canvas canvas, Camera camera) {
        ...
        float thetaHigh = Math.max(theta0, theta1);
        float dt = (thetaHigh - thetaLow)/sampleCount;
        if(dt==0){
            return;
        }
        ...
        this.length = 0;
        float prevX = (float) functionObject.getFunction().evaluateX(thetaLow);
        float prevY = (float) functionObject.getFunction().evaluateY(thetaLow);
        for(float t = thetaLow+dt; t<thetaHigh; t+=dt){
            float x = (float) functionObject.getFunction().evaluateX(t);
            float y = (float) functionObject.getFunction().evaluateY(t);
        ...
        float x = (float) functionObject.getFunction().evaluateX(thetaHigh);
        float y = (float) functionObject.getFunction().evaluateY(thetaHigh);
        canvas.drawLine(prevX, prevY,x,y,paint);
        length+=Math.hypot(x-prevX,y-prevY);
        ...
        float radiusBeyondScreen = 2*Math.max(Math.max(Math.max(Math.abs(camera.getBottom()),Math.abs(camera.getTop())),Math.abs(camera.getRight())),Math.abs(camera.getLeft()));
        canvas.drawLine(0, 0,radiusBeyondScreen*(float)Math.cos(thetaLow),radiusBeyondScreen*(float)Math.sin(thetaLow),paint);
        canvas.drawLine(0, 0,radiusBeyondScreen*(float)Math.cos(thetaHigh),radiusBeyondScreen*(float)Math.sin(thetaHigh),paint);
    }
